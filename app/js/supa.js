...
(function (global) {
  ...
  function signIn(email, password) {
    return netFetch(URL_ + '/auth/v1/token?grant_type=password', {
      method: 'POST',
  ...
  function signOut() {
    ...
    return netFetch(URL_ + '/auth/v1/logout', {
      method: 'POST',
  ...
  function refresh() {
    ...
    refreshing = netFetch(URL_ + '/auth/v1/token?grant_type=refresh_token', {
      method: 'POST',
      headers: { apikey: KEY, 'Content-Type': 'application/json' },
      ...
      /* 인터넷이 끊긴 것뿐이면 세션을 지우지 않는다.
         예전엔 실패 종류를 안 가리고 지워서, 토큰 만료(1시간) 뒤 와이파이가 한 번만
         끊겨도 직원이 이메일·비밀번호를 다시 쳐야 했다. 센터 와이파이는 자주 끊긴다.
         (netFetch 가 붙여주는 e.offline 을 여기서 읽는다) */
      if (!e || !e.offline) save(null);
      throw e;
  ...
  /* 인터넷이 끊기면 fetch 는 서버 응답 없이 그냥 실패한다(TypeError).
     그 경우 friendly() 가 볼 JSON 자체가 없어 "Failed to fetch" 가 화면에 그대로 나갔다.
     여기서 사람 말로 바꿔 던진다 — 센터 와이파이는 실제로 자주 끊긴다. */
  function netFetch(url, opts) {
    return fetch(url, opts).catch(function (e) {
      var err = new Error('인터넷 연결이 끊긴 것 같습니다. 연결을 확인하고 다시 시도해 주세요.');
      err.offline = true;
      err.cause = e;
      throw err;
    });
  ...
  function authed(path, opts, retried) {
    ...
    return token().then(function (t) {
      ...
      return netFetch(URL_ + path, Object.assign({}, opts, { headers: h }));
    }).then(function (res) {
