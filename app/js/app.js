...
(function (global) {
   ...
   *
   * ⚠️ 2026-07-30 문구 교체 — 원래는 이렇게 적혀 있었다:
   *      "사진이 링크 없이도 열릴 수 있는 문제를 발견해, 안전하게 고치는 동안 닫아둡니다"
   *    개발 중에는 정확한 말이었지만, 이 카드는 **센터장이 읽는 자리**다.
   *    시연에서 저 문장이 첫 인상이 되면 보안 위험은 0인데 **사업 위험이 크다.**
   *    (마스터 3종이 같이 지적: PM·보안·개발)
   *
   *    거짓으로 바꾸지는 않았다 — 게이트 R-64·R-65·R-66 이 실제로 안 닫혔고,
   *    그걸 닫은 뒤에 여는 것이 맞다. **사실 그대로, 다만 사고 보고서가 아니라
   *    「신중하게 하고 있다」로 읽히게** 썼다. */
  function renderShareLocked() {
    var c = UI.card();
    c.appendChild(UI.el('p', 'eyebrow', '가족에게 보내기'));
    c.appendChild(UI.el('h2', null, '곧 열립니다'));
    c.appendChild(UI.el('p', 'lede',
      '가족에게 보내는 <b>링크 만들기를 준비하고 있습니다.</b> ' +
      '어르신 사진이 오가는 기능이라, <b>안전 점검을 마친 뒤</b> 엽니다.'));
    c.appendChild(UI.el('div', 'note',
      '올리기와 저장은 지금 그대로 됩니다. 준비가 끝나면 이 자리에 링크 만들기가 나타납니다.'));
  }
  ...
  function renderShareCard() {
    ...
    /* ⚠️ 2026-07-30 정정 — 원래 "폐기하면 링크가 곧바로 막힙니다" 였다.
     *    절반만 맞는 말이었다. 폐기하면 **링크는 그 자리에서 막히지만**,
     *    이미 열려 있던 **사진 주소는 최대 4시간까지 살아 있다**(서명 URL, R-62).
     *    직원이 가족에게 *"지웠어요"* 라고 했는데 사진이 열리면 그게 신뢰 사고다.
     *    보안·PM 마스터가 같이 지적했다. 화면에 사실대로 적는다. */
    c.appendChild(UI.el('p', 'lede',
      '작품을 볼 수 있는 <b>링크</b>를 만들어 가족에게 보내세요. ' +
      '문제가 생기면 <b>폐기</b>할 수 있고, 그러면 그 링크는 곧바로 막힙니다.'));
    c.appendChild(UI.el('div', 'note',
      '폐기해도 <b>이미 열어 둔 사진 화면은 최대 4시간까지</b> 남아 있을 수 있습니다. ' +
      '급하면 가족에게 창을 닫아 달라고 함께 알려 주세요.'));
